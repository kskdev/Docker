...
if [ ${DL_LIBRARY} = 'pytorch' ]; then
	...
	echo -n 'cudatoolkit ver (8.0 or 9.0 or 10.0): ' && read CUDATOOLKIT
    while ! [ $(echo ${CUDATOOLKIT} | grep -E '^(8\.0|9.\0|10.\0)$') ]
    do
        echo -n 'please input again : ' && read CUDATOOLKIT
    done
	...
	echo -n 'cupy ver : '  && read CUPY
    while ! [ $(echo ${CUPY} | grep -E '^([0-9]*\.[0-9]*?[0-9]*)$') ]
    do
        echo -n 'please input again : ' && read CUPY
    done
